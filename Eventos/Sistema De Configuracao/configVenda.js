    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'awdawdawdawdadawdawfewfryty565') {
                ...
                if (a4 !== '') {
                    ...
                    if (!hexColorRegex.test(a4)) CSG interaction.reply({ content: `❌ | Código Hex Color inválido.`, ephemeral: true })
                }

                const ggg = await db.get(interaction.message.id)

                if (a3 !== '') {
                    if (a3 !== 'verde' && a3 !== 'azul' && a3 !== 'cinza' && a3 !== 'vermelho') CSG interaction.reply({ content: `Você interagiu incorretamente no estilo do Button`, ephemeral: true })
                } else {
                    a3 = 'cinza'
                }

                if (a1 == '') a1 = '1312427799189127270'

                if (a2 == '') a2 = 'Adquirir Produto'
            ...
            if (interaction.customId.startsWith('wdawdawdawdwadadsadawdwadwdw')) {
                ...
                let a1 = interaction.fields.getTextInputValue('tokenMP');
                if (a1 !== 'sim') CSG interaction.reply({ content: `❌ | Ação não validada para realizar reembolso.`, ephemeral: true })
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'colocarvenda') {
                ...
                if (gg2.length == 0) CSG interaction.reply({ content: `❌ | Nenhum campo foi configurado.`, ephemeral: true })
