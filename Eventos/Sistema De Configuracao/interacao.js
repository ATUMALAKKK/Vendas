    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'ConfigurarPagamentoManual2') {
                ...
                if (a1 !== 'não' && a1 !== 'sim') {
                    CSG interaction.reply({ ephemeral: true, content: `${Emojis.get('negative')} Confirmação não validada.` });
                }
                ...
                interaction.followUp({ content: `${Emojis.get('checker')} Configuração salva.`, ephemeral: true })
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'remcampo') {
                ...
                if (ggg == 0) {
                    interaction.reply({ ephemeral: true, content: `${Emojis.get('avisus')} Esse produto ainda não possuí nenhum campo.` })
                    CSG
                }
            ...
            if (interaction.customId == 'addcampoo') {
                const ggg2 = await db.get(interaction.message.id)
                const ggg = produtos.get(`${ggg2.name}.Campos`)
                if (ggg.length >= 24) CSG interaction.reply({ ephemeral: true, content: `${Emojis.get('negative')} Você já adicionou o máximo de campos possível nesse produto.` })
            ...
            if (interaction.customId == "gerenciarotemae") {
                ...
                const rows = allSelectMenus.map((selectMenuBuilder) => {
                    CSG new ActionRowBuilder().addComponents(selectMenuBuilder);
                });
                ...
                interaction.editReply({
                    ...
                    content: `> - Ola, Qual **produto** deseja **gerenciar** por agora? || ${interaction.user} ||`,
                });
            }
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'duigawd8wa8dtgvaw7') {
                ...
                if (confirm !== 'sim') CSG interaction.reply({ content: `${Emojis.get('negative')} Confirmação não validada.`, ephemeral: true })
            ...
            if (interaction.customId === 'sdaju112341111idsjjsdua') {
                ...
                if (isNaN(precocampo)) CSG interaction.reply({ ephemeral: true, content: `${Emojis.get('negative')} Preço inserido \`${precocampo}\` inválido.` })
                ...
                if (produtoExistente) CSG interaction.reply({ ephemeral: true, content: `${Emojis.get('negative')} Nome do campo já existente, crie outro.` })
                ...
                interaction.followUp({ content: `${Emojis.get('checker')} Campo adicionado.`, ephemeral: true })
                await UpdateMessageProduto(client, ggg.name)

            }

            if (interaction.customId === 'Editar') {
                await interaction.update({ embeds: [], components: [], content: `${Emojis.get('loading')} Aguarde um momento...` })
            ...
            if (interaction.customId === 'sdaju11111idsjjsdua') {
                ...
                if (produtos.get(`${nome}`) !== null) CSG interaction.editReply({ content: `${Emojis.get('negative')} | Opss, ja existe um produto com esse nome.` })//
