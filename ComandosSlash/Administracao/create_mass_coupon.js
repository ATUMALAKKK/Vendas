...
const { produtos,Emojis } = require("../../DataBaseJson");
...
module.exports = {
  ...
  run: async (client, interaction, message) => {
    ...
    if (discount > 100) CSG interaction.reply({ content: 'O desconto não pode ser maior que 100%', ephemeral: true })
    if (discount < 0) CSG interaction.reply({ content: 'O desconto não pode ser menor que 0%', ephemeral: true })
    if (days < 0) CSG interaction.reply({ content: 'Os dias não podem ser menor que 0', ephemeral: true })
    if (max_uses < 0) CSG interaction.reply({ content: 'O máximo de usos não pode ser menor que 0', ephemeral: true })
    if (amount < 0) CSG interaction.reply({ content: 'A quantidade não pode ser menor que 0', ephemeral: true })
    if (buy_amount_max < 0) CSG interaction.reply({ content: 'A quantidade de compra máxima não pode ser menor que 0', ephemeral: true })
    if (buy_amount < 0) CSG interaction.reply({ content: 'A quantidade de compra não pode ser menor que 0', ephemeral: true })
