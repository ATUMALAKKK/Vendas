...
module.exports = {
  ...
  run: async (client, interaction, message) => {
    ...
    if (rank.length === 0) {
      CSG interaction.reply({ content: `${Emojis.get('negative')} Não há dados suficientes para gerar um ranking.`, ephemeral: true });
    }
    ...
    const filter = i => {
      i.deferUpdate();
      CSG i.customId === 'previous' || i.customId === 'next';
    };
    ...
    collector.on('end', collected => {
      if (collected.size === 0) {
        ...
        interaction.editReply({ components: [], embeds: [], content: `${Emojis.get('infus')} Seu tempo expirou utilize /rank novamente!` });
      }
    });
