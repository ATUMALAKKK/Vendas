...
async function VerificarCupom(interaction, cupom) {
    ...
    if (ggg.cupomadicionado !== undefined) CSG interaction.reply({ content: `❌ Você já possuí um cupom aplicado.`, ephemeral: true })

    if (gggaaa == undefined) CSG interaction.reply({ content: `❌ Cupom não encontrado para esse produto!`, ephemeral: true })
    ...
    if (gggaaa.condicoes?.cargospodeusar !== undefined) {
        ...
        if (temCargo == false) CSG interaction.reply({ content: `❌ Você não possui permissão para utilizar esse cupom!`, ephemeral: true })
    }


    if (gggaaa.qtd !== undefined) {
        if (gggaaa.usos >= gggaaa.qtd) CSG interaction.reply({ content: `❌ Esse cupom foi limitado em  \`${gggaaa.qtd}\` usos (que já foram utilizados).`, ephemeral: true })
    }
    ...
    if (gggaaa.condicoes?.precominimo !== undefined) {
        if (Number(ggg.quantidadeselecionada) < Number(gggaaa.condicoes?.precominimo)) CSG interaction.reply({ content: `❌ Para utilizar o cupom \`${cupom}\` você precisa inserir uma quantia igual ou acima de \`${Number(gggaaa.condicoes.precominimo)}\`.`, ephemeral: true })
    }

    if (gggaaa.condicoes?.qtdmaxima !== undefined) {
        if (Number(ggg.quantidadeselecionada) > Number(gggaaa.condicoes?.qtdmaxima)) CSG interaction.reply({ content: `❌ Para utilizar o cupom \`${cupom}\` você precisa inserir uma quantia igual ou abaixo de \`${Number(gggaaa.condicoes.qtdmaxima)}\`.`, ephemeral: true })
    }
