...
async function MessageCreate(interaction, client) {
    ...
    if (fdfd.textobutton == undefined) {
        ...
        if (yyy.Campos.length <= 1) CSG interaction.update({ content: `Faça o processo de envio novamente!`, ephemeral: true, components: [] })
        ...
        try {
            await interaction.update({ embeds: [], components: [], content: `${Emojis.get('loading')} Aguarde um momento...` }).then(async msgg => {
                ...
                await channel.send({ embeds: [embed], components: [style2row] }).then(async msggg => {
                    ...
                    await msgg.edit({ content: `${Emojis.get('checker')} Postado com sucesso!`, ephemeral: true, components: [row4] })
            ...
            await interaction.followUp({ content: `${Emojis.get('negative')} Ocorreu um erro ao enviar a mensagem.\n\`${mensagem}\``, ephemeral: true })
        ...
        try {
            await interaction.update({ embeds: [], components: [], content: `${Emojis.get('loading')} Aguarde um momento...` }).then(async msgg => {
                ...
                await channel.send({ embeds: [embed], components: [row2] }).then(msggg => {
                    ...
                    msgg.edit({ content: `${Emojis.get('checker')} Postado com sucesso!`, ephemeral: true, components: [row4] })
                })
            ...
            interaction.followUp({ content: `${Emojis.get('negative')} Ocorreu um erro ao enviar a mensagem.\n\`${mensagemTraduzida}\``, ephemeral: true })
        }
...
async function UpdateMessageProduto(client, produto) {
    ...
    if (ghgh.Campos.length > 1) {
        ...
        if (ghgh.Campos[0] == undefined) {
            if (ghgh.mensagens == undefined) CSG produtos.set(`${produto}.mensagens`, [])
            for (let iiiiii = 0; iiiiii < ghgh.mensagens.length; iiiiii++) {
                const element = ghgh.mensagens[iiiiii];
                const channel = await client.channels.fetch(element.channelid)
                const fetchedMessage = await channel.messages.fetch(element.mesageid);
        ...
        if (ghgh.mensagens?.length == undefined) CSG
        if (ghgh.mensagens?.length == 0) CSG
        for (let iiiiii = 0; iiiiii < ghgh.mensagens.length; iiiiii++) {
            const element = ghgh.mensagens[iiiiii];
                    ...
                    row2 = new ActionRowBuilder()
                        .addComponents(
                            new ButtonBuilder()
                                ...
                                .setLabel(`Adquirir Produto`)
                                .setEmoji(`1312427799189127270`)
                                .setStyle(2),)
                } else {
                    row2 = new ActionRowBuilder()
                        .addComponents(
