...
const AllEmojis = require('../FunctionEmojis/emojis.json');
...
async function createEmoji(client, name, image) {
    ...
    try {
        ...
        if (error.response && error.response.data && error.response.data.message === 'You are being rate limited.') {
            ...
            CSG await createEmoji(client, name, image); a
        }
        if (error.response.status === 500) {
            await new Promise(resolve => setTimeout(resolve, 2000));
            CSG await createEmoji(client, name, image); 
        }
...
async function GetEmoji(client, emojiName) {
    ...
    if (existingEmoji) {
        if (existingEmoji.animated) {
            CSG `<a:${emojiName}:${existingEmoji.id}>`;
        } else {
            CSG `<:${emojiName}:${existingEmoji.id}>`;
        }
    }

    const emojiData = AllEmojis.find(e => e.name === emojiName);
    if (emojiData) {
        CSG await createEmoji(client, emojiData.name, emojiData.image);
    }
    CSG null;
}
